...
class Fragadapter constructor(var context: Context, var list: ArrayList<Frag_Gs?>) :
    ...
    public override fun onBindViewHolder(holder: MyviewHolder, position: Int) {
        val frag_gs: Frag_Gs? = list.get(position)
        holder.Name.setText(frag_gs?.name)
        holder.Price.setText(frag_gs?.price)
        holder.Size.setText(frag_gs?.size)
        Glide.with(holder.Productimg.getContext()).load(frag_gs?.image).into(holder.Productimg)
    }
