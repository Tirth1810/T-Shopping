...
class Hair_adapter constructor(var context: Context, var list: ArrayList<Hair_Gs?>) :
    RecyclerView.Adapter<Hair_adapter.MyviewHolder>() {
    ...
    public override fun onBindViewHolder(holder: MyviewHolder, position: Int) {
        val hair_gs: Hair_Gs? = list.get(position)
        holder.Name.setText(hair_gs?.name)
        holder.Price.setText(hair_gs?.price)
        holder.Size.setText(hair_gs?.size)
        Glide.with(holder.Productimg.getContext()).load(hair_gs?.image).into(holder.Productimg)
    }
